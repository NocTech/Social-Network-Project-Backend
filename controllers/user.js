            ...
            bcrypt.compare(password, user.password, (err, check) => {
                if (check) {
                    ...
                    if (params.gettoken) {
                        ...
                        // This is a security measure to avoid sending passwords to the client
...
function getUser(req, res) {
    ...
    User.findById(userId, (err, user) => {
        ...
        followThisUser(req.user.sub, userId).then((value) => {
            //WARNING SECURITY MEASURE: DONT SEND USERS PASSWORDS WITH REQUEST BACK TO THE CLIENT 
            // This is a security measure to avoid sending passwords to the client
            user.password = undefined;
            return res.status(200).send({
                user,
...
function getUsers(req, res) {
    ...
    User.find().sort('_id').paginate(page, itemsPerPage, (err, users, total) => {
        ...
        //WARNING SECURITY MEASURE: DONT SEND USERS PASSWORDSWITH REQUEST BACK TO THE CLIENT
        // This is a security measure to avoid sending passwords to the client
        users.forEach((user) => {
            user.password = undefined;
        });
